      ...
      INCLUDE 'fft.dim'
c
      integer p,q,kx,tmp, jiter
      character(len=11) :: filename
      character(len=11) :: matfname
...
c      open(211, file ='sg1.out', status='unknown')
c      open(212, file ='sg2.out', status='unknown')
c      open(213, file ='sg3.out', status='unknown')
c      open(214, file ='sg4.out', status='unknown')
c      open(215, file ='sg5.out', status='unknown')
c      open(216, file ='sg6.out', status='unknown')
c      open(217, file ='sg7.out', status='unknown')
c      open(218, file ='sg8.out', status='unknown')
...
C ccccccccccccccccccccccccccccFILES BEING WRITTEN IN EACH ITERccccccccccccccccccccccccccccccccccccccc
       write(*,*) 'writing s11 component for this iteration...'
...
c       write(cmdline, "(A52,A11,A7)") 'matlab -nodisplay -nosplash
c    & -nodesktop -r "infile=''', filename,''';test"'
        write(cmdline, "(A26,A11,A11)") 'matlab -nojvm -r "infile='''
     &, filename,''';fortran_int"'
...
cccccccccccccccccccccc here read back the matlab approximation into sg cccccccccccccccccccc

        write(*,*)'read approx'
          do kz=1,npts3
      
                 tmp = kz+100
                 write(matfname, '("SG",I3,I1,I1,".out")') tmp,iii,jjj
                 write(*,*)matfname
                  open(unit=kz+1000,file=matfname,status='old')
                 write(*,*) matfname
               do yy=1,npts2
                    do xx=1,npts1
                            read(kz+1000,*) sgdata(xx,yy)

               
                             sg(iii,jjj,kx,ky,kz)= sgdata(xx,yy)
                      enddo
                 enddo
...
cccccccccccccccccccccccccccccccccccAnuva edits
...
ccc upto this point, ddisgrad and ddisgradim are very much comparable and 10e-7 or -8 range
...
c      write(9000,*) 'strain='
c      write(9000,*)  disgrad
c      write(9000,*)  'end iter'
c     SYMMETRIZATION
c

      ...
      INCLUDE 'fft.dim' c integer p,q,kx,tmp, jiter character(len=11) ::
filename character(len=11) :: matfname

c dimension data(2*npts1*npts2*npts3),nn(3),nn2(2) dimension xk(3),xk2(3)
dimension a(3,3),g1(3,3,3,3)
c
      dimension aux6(6),aux33(3,3)
      dimension spart(npts1,npts2)
      dimension sgdata(npts1,npts2)
      dimension cdata(npts1,npts2)
      dimension convout(3,3,npts1,npts2,npts3)
...
c
      call cpu_time(start)
c                  ! put code to test here
       pi=4.*atan(1.)
c
      open(55,file='vm.out',status='unknown')
      open(5000,file='fftgamma.out',status='unknown')

      open(9000,file='strain.out',status='unknown')

       open(500, file ='c0initial.out', status='unknown')
       open(700,file = 's0.out', status = 'unknown')
       open(5000,file='cg.out',status='unknown')
       open(1111,file='nn.out',status = 'unknown')
       open(1112,file='s0.out',status = 'unknown')
cc      open(35,file='ac tiv.out',status='unknown')
...
ccccccccccccc   check  cccccccccccc
c       write(*,*) 'disgradmacro='
c       write(*,*) disgradmacro
        write(5000,*) 'cg ='
        write(5000,*) cg
...
cc      err2mod=2*error
      erre=2*error
      errs=2*error
cccccccccccccccccccccccccccccc start iteration ccccccccccccccccccccccc
      do while(iter.lt.itmax.and.(errs.gt.error.or.erre.gt.error))
c     c      do while(iter.lt.itmax.and.erre.gt.error)

        if(iter.eq.10) then
C ccccccccccccccccccccccccccccFILES BEING WRITTEN IN EACH ITERccccccccccccccccccccccccccccccccccccccc
      ...
      write(*,*) kz
                  do iii=1,3
                   do  jjj=1,3
                     ...
                     do ky=1,npts2
                      do kx= 1,npts1
                        spart(kx,ky) = sg(iii,jjj,kx,ky,kz)

                       enddo
                     enddo
                     tmp = kz+100
                     write(filename,'("SG",I3,I1,I1,".out")')tmp,iii,jjj
                     open(unit=kz+1000,file=filename,status='unknown')

                       do yy=1,npts2
                        do xx=1,npts1
                             write(kz+1000,*) spart(xx,yy)
                        enddo
                       enddo
                 ...
                 enddo
              enddo
            ! write command for matlab

      enddo 


 
        write(cmdline, "(A26,A11,A7)") 'matlab -nojvm -r "infile='''
     &, filename,''';fast"'
...
cccccccccccccccccccccc here read back the matlab approximation into convout cccccccccccccccccccc
        ...
        write(*,*)'read approx'
          do kz=1,npts3
                 ...
                 do iii=1,3
                    do jjj=1,3
                 write(matfname, '("WC",I3,I1,I1,".out")') tmp,iii,jjj
                 write(*,*)matfname
                 open(unit=kz+1000,file=matfname,status='old')
                 write(*,*) matfname
               do yy=1,npts2
                    do xx=1,npts1
                            read(kz+1000,*) cdata(xx,yy)

               
                             convout(iii,jjj,kx,ky,kz)= cdata(xx,yy)
                      enddo
                 enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      write(5000,*) 'iter=',iter

      write(9000,*) 'iter=',iter
...
cccccccccccccc convout is added to get new strain cccccccccccccccc
...
c      write(*,*) 'REAL PART =',kxx,kyy,kzz,data(k1)
     ...
     #  ddisgradmacro(m,n)+convout(m,n,kxx,kyy,kzz)
...
cccccccccccccc for each entry of sigma(i,j) this is done:
      do 5 k=1,npts3
      do 5 j=1,npts2
      do 5 i=1,npts1
c
...
ccccccccccccc the contraction cccccccccccccccccccccc
...
ccc upto this point, ddisgrad and ddisgradim are very much comparable and 10e-7
or -8 range
ccccccccccccccccccccccccccc all the gammas in the iter have been printed
1     continue
...
cccccccccccccccccccc after this ifft, the im part suddenly drops to 10e-14
...
!        WRITE(*,*)      modex,nsmx,nrsx,gamd0x,twshx,isectwx
        READ(UR1,*)     tau0xf,tau0xb,tau1x,thet0x,thet1x
!        write(*,*)      tau0xf,tau0xb,tau1x,thet0x,thet1x
